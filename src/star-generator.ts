...
function populateStars(minX: number, maxX: number, minY: number, maxY: number,
...
function translateY(deltaY: number): void {
    for (let i = 0; i < numStars; i++) {
        starYs[i] += deltaY;
    }
    cleanStars();
    populateStars(-univXBound, univXBound, -univYBound, deltaY - univYBound,
        univMinZ, univMaxZ);
}

function updateUnivXBound(newUnivXBound: number): void {
    if (newUnivXBound === univXBound) return;
    if (newUnivXBound < univXBound) {
        univXBound = newUnivXBound;
        cleanStars();
    } else {
        populateStars(-newUnivXBound, -univXBound, -univYBound, univYBound,
            univMinZ, univMaxZ);
        populateStars(univXBound, newUnivXBound, -univYBound, univYBound,
            univMinZ, univMaxZ);
        univXBound = newUnivXBound;
    }
}

function updateUnivYBound(newUnivYBound: number): void {
    if (newUnivYBound == univYBound) return;
    if (newUnivYBound < univYBound) {
        univYBound = newUnivYBound;
        cleanStars();
    } else {
        populateStars(-univXBound, univXBound, -newUnivYBound, -univYBound,
            univMinZ, univMaxZ);
        populateStars(-univXBound, univXBound, univYBound, newUnivYBound,
            univMinZ, univMaxZ);
        univYBound = newUnivYBound;
    }
}

function drawStarsOffscreen(): void {
    for (let i = 0; i < numStars; i++) {
        ...
        ctxOfs.beginPath();
        ctxOfs.arc(starX + windowXBound, starY + windowYBound, radius,
            0, 2 * Math.PI);
        ctxOfs.fill();
    }
}
...
document.addEventListener("DOMContentLoaded", function () {
    canvasElt = <HTMLCanvasElement>document.getElementById("background-canvas");
