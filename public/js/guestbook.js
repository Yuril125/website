...
function guestbookDate(timestamp) {
...
async function updateGuestbook() {
    ...
    if (guestbook === undefined) {
        ...
        document.getElementById('main')?.appendChild(document.createTextNode('Fetching guestbook from server failed :( Please reload to try again'));
        return false;
    }
    ...
    const guestbookBoxes = guestbook.map(function (entry) {
        ...
        timeDiv.appendChild(document.createTextNode(guestbookDate(entry.timestamp)));
        ...
        if (entry.websiteContact) {
            ...
            contactDiv.appendChild(document.createTextNode('Website/Contact: ' + entry.websiteContact));
            guestbookHeader.appendChild(contactDiv);
        }
        const messageDiv = document.createElement('div');
        messageDiv.className = 'message';
        messageDiv.appendChild(document.createTextNode(entry.messageBody));
        guestbookBox.replaceChildren(guestbookHeader, document.createElement('hr'), messageDiv);
        return guestbookBox;
    });
    document.getElementById('guestbook-entries')?.replaceChildren(...guestbookBoxes);
    return true;
}
async function submitHandler(event) {
    event.preventDefault();
    const nameElt = document.getElementById('entry-name');
    const contactElt = document.getElementById('entry-contact');
    const msgElt = document.getElementById('entry-body');
    const entryData = {
        name: nameElt.value,
    ...
    try {
        ...
        if (response.status === 201) {
            ...
            submissionStatusElt?.replaceChildren(document.createTextNode('Submission successful!'));
            setTimeout(updateGuestbook, 500);
        }
        else {
            const errorMsg = await response.text();
            submissionStatusElt?.replaceChildren(document.createTextNode('Server error :('));
            console.error('Server responded with:', response.status, errorMsg);
        }
    }
    catch (error) {
        submissionStatusElt?.replaceChildren(document.createTextNode(`:( Error submitting the form: ${error} \u2014 Please try again`));
